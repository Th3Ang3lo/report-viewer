...
<template>
  <div v-if="!fileSelected" class="max-w-md mx-auto h-40 rounded-lg border-2 border-dashed flex items-center justify-center">
    <label :htmlFor="props.name" class="cursor-pointer text-center p-4 md:p-8" tabindex="1">
    ...
    <input :id="props.name" :name="props.name" type="file" class="hidden" ref="fileInput" @change="handleSelectFile" />
  </div>
